...
@generated function blockindex2global(block_sizes::BlockSizes{N}, block_index::BlockIndex{N}) where {N}
    ex = Expr(:tuple, [:(cumulsize(block_sizes, $k, block_index.I[$k]) + block_index.α[$k] - 1) for k = 1:N]...)
    return quote
        $(Expr(:meta, :inline))
        @inbounds v = $ex
        return $ex
...
@inline function blockindex2global(block_sizes::BlockSizes{1}, block_index::BlockIndex{1})
    @inbounds v =(cumulsize(block_sizes, 1, block_index.I[1]) + block_index.α[1] - 1,)
    return v
end

@inline function blockindex2global(block_sizes::BlockSizes{2}, block_index::BlockIndex{2})
    @inbounds v =(cumulsize(block_sizes, 1, block_index.I[1]) + block_index.α[1] - 1,
                  cumulsize(block_sizes, 2, block_index.I[2]) + block_index.α[2] - 1)
    return v
end
...
@inline function blockindex2global(block_sizes::BlockSizes{3}, block_index::BlockIndex{3})
    @inbounds v =(cumulsize(block_sizes, 1, block_index.I[1]) + block_index.α[1] - 1,
                  cumulsize(block_sizes, 2, block_index.I[2]) + block_index.α[2] - 1,
                  cumulsize(block_sizes, 3, block_index.I[3]) + block_index.α[3] - 1)
    return v
end
